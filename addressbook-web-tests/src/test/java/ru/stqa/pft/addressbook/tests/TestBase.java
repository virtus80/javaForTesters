...
public class TestBase {
  ...
  public void verifyContactListInUI() {
    if (Boolean.getBoolean("verifyUI")) {
      ...
      Contacts uiContacts = app.contact().getContacts();
      assertThat(uiContacts, equalTo(dbContacts.stream().map((c) -> new ContactData().withId(c.getId()).
              withFirstName(c.getFirstName()).withLastName(c.getLastName()).withAddress(c.getAddress()).withEmail(c.getEmail()).
              withMobilePhone(c.getMobilePhone())).collect(Collectors.toSet())));
    }
